  ...
  // valid user found
   ...
   if( ( $level->ChkLevel( $_SESSION['token'] ) === "root" ) || ( $level->ChkLevel( $_SESSION['token'] === "user" ) ) ) {
    ...
    $dbconn = $db->dbConnect( $defined['dbhost'], $defined['username'], $defined['password'], $defined['dbname'] );
				...
				if( $group === "admin" ) {
 				$query = "SELECT * FROM `conf_pxe_groups` ORDER BY `pxe-group-name` ASC";
				} else {
				 $query = "SELECT * FROM `conf_pxe_groups` WHERE `group` = \"" . $group . "\" ORDER BY `pxe-group-name` ASC";
				}
				if( ( $value = $db->dbQuery( $val->ValidateSQL( $query, $dbconn ), $dbconn ) ) !== -1 ) {
     $pxedata = $db->dbArrayResults( $value );
    }

    // are there subnets? if so populate sql queries to look up hosts per subnet
				if( count( $pxedata ) !== 0 ) {
 				foreach( $pxedata as $key => $value ) {
					 if( $group === "admin" ) {
  				 $sql[$value['pxe-group-name']] = "SELECT * FROM `conf_hosts` WHERE `pxe-group` = \"" . $value['pxe-group-name'] . "\"";
						} else {
  				 $sql[$value['pxe-group-name']] = "SELECT * FROM `conf_hosts` WHERE `pxe-group` = \"" . $value['pxe-group-name'] . "\" AND `group` = \"" . $group . "\"";
						}
 				}
